...
export class AuthService {
  ...
  constructor( private cookieService: CookieService,
    ...
    async register(formData: REGISTER_FORM_DATA) {
      ...
      const httpData: HTTP_REQ = { url: 'register', body: {...formData,role:1} };
      ...
      if (success && data?.accessToken) {
        ...
        console.log(error?.message || 'Failure during register')
    ...
    async login(formData: LOGIN_FORM_DATA) {
      ...
      if (success && data?.accessToken) {
        ...
        console.log(error?.message || 'Failure during login')
      }
      ...
      if (success && data?.length > 0) {
        ...
        console.log(error?.message || 'Failure during get profile')
        return null;
      }
    }
    // LOGOUT
